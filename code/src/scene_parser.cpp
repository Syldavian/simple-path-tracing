...
Material *SceneParser::parseMaterial() {
    ...
    auto *answer = new Material(diffuseColor, emissionColor, reflection, shininess);
    return answer;
}
...
Sphere *SceneParser::parseSphere() {
    ...
    if (!strcmp(token, "texture")) {
        getToken(token);
        image = Image::LoadPPM(token);
        cout<<"texture image size:"<<image->Height()<<" "<<image->Width()<<endl;
    ...
    return new Sphere(center, radius, current_material, image, tex);
}
...
Plane *SceneParser::parsePlane() {
    ...
    assert (!strcmp(token, "}"));
    assert (current_material != nullptr);
    return new Plane(normal, offset, current_material, image, tex);
}
...
Mesh *SceneParser::parseTriangleMesh() {
    ...
    Mesh *answer = new Mesh(filename, current_material, scale, offset);
    return answer;
}
...
RevSurface *SceneParser::parseRevSurface() {
    ...
    assert (!strcmp(token, "}"));
    auto *answer = new RevSurface(profile, current_material, image, tex);
    return answer;
}
...
Transform *SceneParser::parseTransform() {
    ...
    assert(object != nullptr);
    assert (!strcmp(token, "}"));
    return new Transform(matrix, object,tex);
}
